...
class VeluxTool extends CommandLineTool {
          ...
          .on('request', (request) => {
            ...
            if (request.data != null) {
              this.vdebug(
                ...
                toHexString(request.cmd, 4), request.data.toString('hex').toUpperCase()
              )
            }
          })
          .on('response', (response) => {
            if (response.data != null) {
              this.vdebug(
                '%s: request %d: %s [%s]: %s', name, response.request.id,
                response.cmdName, toHexString(response.cmd, 4),
          ...
          .on('notification', (notification) => {
            if (notification.data != null) {
              this.vdebug(
                ...
                notification.data.toString('hex').toUpperCase()
              )
            }
            this.debug(
              '%s: %s [%s]', name, notification.cmdName,
              toHexString(notification.cmd, 4)
            )
          })
          .on('send', (data) => {
            this.vvdebug('%s: send %s', name, data.toString('hex').toUpperCase())
          })
          .on('data', (data) => {
            this.vvdebug('%s: received %s', name, data.toString('hex').toUpperCase())
          })
      }
  ...
  async info (...args) {
    ...
    this.print(await this.client.gwGetVersion())
    this.print(await this.client.gwGetProtocolVersion())
    this.print(await this.client.gwGetState())
    this.print(this.jsonFormatter.stringify(await this.client.gwCsGetSystemtableData()))
    await timeout(1000)
    this.print('goodbye')
