...
class VeluxTool extends CommandLineTool {
      ...
      if (clargs.command === 'discover') {
        ...
        this.client
          ...
          .on('error', (error) => {
            ...
            if (error.request.args == null) {
              this.debug(
                '%s: request %d: %s', name, error.request.id,
                error.request.cmdName.slice(0, -4)
              )
            } else {
              this.debug(
                '%s: request %d: %s %j', name, error.request.id,
                error.request.cmdName.slice(0, -4), error.request.args
              )
            }
            this.warn(
          ...
          .on('request', (request) => {
            if (request.args == null) {
              this.debug(
                '%s: request %d: %s', name, request.id,
                request.cmdName.slice(0, -4)
              )
            } else {
              this.debug(
                '%s: request %d: %s %j', name, request.id,
                request.cmdName.slice(0, -4), request.args
              )
            }
          ...
          .on('notification', (notification) => {
            if (notification.data != null) {
              this.vdebug(
                '%s: %s [%s]: %s', name, notification.cmdName,
  ...
  async info (...args) {
    ...
    this.print(await this.client.GW_GET_VERSION_REQ())
    this.print('api v' + await this.client.GW_GET_PROTOCOL_VERSION_REQ())
    this.print(await this.client.GW_GET_STATE_REQ())
    try {
      this.print(await this.client.GW_LEAVE_LEARN_STATE_REQ())
    } catch (error) {}
    this.print(await this.client.GW_SET_UTC_REQ())
    this.print('time: ' + (await this.client.GW_GET_LOCAL_TIME_REQ()).toISOString())
    this.print(this.jsonFormatter.stringify(await this.client.GW_CS_GET_SYSTEMTABLE_DATA_REQ()))
    this.print(await this.client.GW_HOUSE_STATUS_MONITOR_ENABLE_REQ())
    // this.print(this.jsonFormatter.stringify(await this.client.GW_GET_NODE_INFORMATION_REQ(0)))
    // this.print(this.jsonFormatter.stringify(await this.client.GW_GET_NODE_INFORMATION_REQ(1)))
    this.print(this.jsonFormatter.stringify(await this.client.GW_GET_ALL_NODES_INFORMATION_REQ()))
    this.print(await this.client.GW_STATUS_REQUEST_REQ(0))
    this.print(await this.client.GW_STATUS_REQUEST_REQ(1))
    await timeout(5000)
    this.print('goodbye')
