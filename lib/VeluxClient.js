...
// To be able to send multiple requests in parallel:
// - async GW_..._REQ() needs to return on corresponding GW_..._CFM
//   indicating session or request ID?
// - on end GW_...__NTF, emit event to client with session or request ID?
//   or simply send state change events, never mind the originiation?
...
class VeluxClient extends EventEmitter {
  ...
  */
  ...
  /** Send GW_HOUSE_STATUS_MONITOR_ENABLE_REQ command.
    */
  async GW_HOUSE_STATUS_MONITOR_ENABLE_REQ () { // 8.2.1
    return this.#send(commands.GW_HOUSE_STATUS_MONITOR_ENABLE_REQ)
  }

  _GW_HOUSE_STATUS_MONITOR_ENABLE_CFM (data) { // 8.2.2
    this.emit('_' + commands.GW_HOUSE_STATUS_MONITOR_ENABLE_REQ)
  }

  /** Send GW_HOUSE_STATUS_MONITOR_DISABLE_REQ command.
    */
  async GW_HOUSE_STATUS_MONITOR_DISABLE_REQ () { // 8.2.1
    return this.#send(commands.GW_HOUSE_STATUS_MONITOR_DISABLE_REQ)
  }

  _GW_HOUSE_STATUS_MONITOR_DISABLE_CFM (data) { // 8.2.2
    this.emit('_' + commands.GW_HOUSE_STATUS_MONITOR_DISABLE_REQ)
  }

  /** Send GW_GET_NODE_INFORMATION_REQ command.
    * @param {integer} nodeId - The node ID.
    */
  async GW_GET_NODE_INFORMATION_REQ (nodeId) { // 8.3.1
    const data = Buffer.allocUnsafe(1)
    data.writeUInt8(nodeId)
    return this.#send(commands.GW_GET_NODE_INFORMATION_REQ, data, { nodeId })
  }
  ...
  _NODE_INFORMATION_NTF (data) {
    this._timestamp = data.readUint32BE(99) & 0xFFFF0000
    return {
  ...
  _GW_NODE_INFORMATION_CHANGED_NTF (data) { // 8.3.8
    const response = {
      nodeId: data.readUInt8(0),
  ...
  _GW_NODE_STATE_POSITION_CHANGED_NTF (data) { // 8.3.9
    const response = {
      nodeId: data.readUInt8(0),
      state: data.readUInt8(1),
      currentPosition: parsePosition(data.readUInt16BE(2)),
      targetPosition: parsePosition(data.readUInt16BE(4)),
      fp1Position: parsePosition(data.readUint16BE(6)),
      fp2Position: parsePosition(data.readUint16BE(8)),
      fp3Position: parsePosition(data.readUint16BE(10)),
      fp4Position: parsePosition(data.readUint16BE(12)),
      remainingTime: data.readUint16BE(14),
      // timeStamp: new Date(data.readUint32BE(16) * 1000).toISOString()
      timeStamp: new Date(
        (this._timestamp + (data.readUint16BE(16))) * 1000
      ).toISOString()
    }
    console.log(response)
  }
  ...
  async GW_STATUS_REQUEST_REQ (nodeId) { // 10.3.1
    const data = Buffer.allocUnsafe(26).fill(0)
    data.writeUInt16BE(++this._session, 0)
    data.writeUInt8(1, 2) // one node ID
    data.writeUInt8(nodeId, 3)
    // data.writeUInt8(nodeId[1], 4)
    data.writeUInt8(3, 23) // request main info
    return this.#send(
      commands.GW_STATUS_REQUEST_REQ, data, { nodeId }, '_s' + this._session
    )
  }
