...
const commands = Object.freeze({
  ...
  GW_PASSWORD_ENTER_REQ: { // 5.1.1 - Enter password to authenticate request
    id: 0x3000,
    encode: (params) => {
      OptionParser.toObject('params', params)
      OptionParser.toString('params.password', params.password, true)
  ...
  GW_PASSWORD_CHANGE_REQ: { // 5.1.3 - Request password change.
    ...
    encode: (params) => {
      OptionParser.toObject('params', params)
      OptionParser.toString('params.oldPassword', params.oldPassword, true)
      OptionParser.toString('params.newPassword', params.newPassword, true)
  ...
  GW_SET_NETWORK_SETUP_REQ: { // 6.9.1 - Set network parameters.
    ...
    encode: (params) => {
      OptionParser.toObject('params', params)
      OptionParser.toHost('params.address', params.address)
      OptionParser.toHost('params.mask', params.mask)
      OptionParser.toHost('params.gateway', params.gateway)
      OptionParser.toBool('params.dhcp', params.dhcp)
  ...
  GW_GET_GROUP_INFORMATION_NTF: { // 8.4.3 - Acknowledge to GW_GET_NODE_INFORMATION_REQ.
    id: 0x0230,
    req: 0x0220 // GW_GET_GROUP_INFORMATION_REQ
  },
  ...
  GW_COMMAND_SEND_REQ: { // 10.1.1 - Send activating command direct to one or more io-homecontrol® nodes.
    ...
    encode: (params) => {
      ...
      OptionParser.toArray('params.nodeIds', params.nodeIds)
  ...
  GW_COMMAND_SEND_CFM: { // 10.1.2 - Acknowledge to GW_COMMAND_SEND_REQ.
    id: 0x0301,
    // req: 0x0300, // GW_COMMAND_SEND_REQ
    decode: (data) => {
  ...
  GW_COMMAND_RUN_STATUS_NTF: { // 10.1.3 - Gives run status for io-homecontrol® node.
    ...
    decode: (data) => {
      ...
      return {
        ...
        position: decodePosition(data.readUInt16BE(5)),
  ...
  GW_STATUS_REQUEST_CFM: { // 10.3.2 - Acknowledge to GW_STATUS_REQUEST_REQ.
    id: 0x0306,
    req: 0x0305, // GW_STATUS_REQUEST_REQ
    sessionId: true,
    decode: (data) => {
      return decodeSessionStatus(data)
    }
  },
  GW_STATUS_REQUEST_NTF: { // 10.3.3 - Acknowledge to GW_STATUS_REQUEST_REQ. Status request from one or more io-homecontrol® nodes.
    id: 0x0307,
    req: 0x0305, // GW_STATUS_REQUEST_REQ
    decode: (data) => {
      checkData(data, 18)
